...
#####################################
### Bayesian Sample Size Calculations - Using Frequentist Operating Characteristics - Very Pessimistic Analysis Prior
#####################################
...
#        20 FEB 2020 - added optimisitc prior specification for sample size simulations (use same seed as for uninformative prior)
...
output_name = "BayesSampleSizeCalcs_freqCritOpsstat_N1000_verypessimmisticprior.RData"
...
benchmarking_pessimistic.mod <- stan_model(file = path(getwd(),"02_SampleSizeCalcs","05_VeryPessimistic","05_benchmarking_verypessimistic.stan",ext=""))   ## C++ Compilation
## TODO - add pessimistic and optimistic estimation models here! 
...
# Model Fitting 
lm.est <- sampling(benchmarking_pessimistic.mod,    # compiled model
...
res=lapply(X=dat,FUN=decision_func,stanMod=benchmarking_pessimistic.mod,bnmk=bnmk,cert_thresh=cert_thresh) 
...
power_res=lapply(X=powerdat,FUN=decision_func,stanMod=benchmarking_pessimistic.mod,bnmk=bnmk,cert_thresh=cert_thresh) 
...
write.csv(type1err,file=path(outpath,"02_Type1Err","type1err_verypessimistic.csv",ext=""))
write.csv(freq_power,file=path(outpath,"01_Power","power_verypessimistic.csv",ext=""))
